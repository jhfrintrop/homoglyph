...
is_fast = True  # If True, then it runs on a very small dataset (and results won't be that great)
...
def create_output():
    ...
    for i in range(max_epochs):
        ...
        scores = [
            -x[0]
            for x in model.predict([X1_valid, X2_valid])
        ]

        t_auc = roc_auc_score(y_valid, scores)
        if t_auc > max_auc:
            print('Updated best AUC from {:f} to {:f}'.format(max_auc, t_auc))
            max_auc = t_auc
            max_idx = i + 1
    ...
    scores = [
        -x[0]
        for x in model.predict([X1_test, X2_test])
    ]
    ...
    # Run Edit distance
    scores = [
        (editdistance.eval(x[0].lower(), x[1].lower()), len(x[0]), 1.0 - x[2])
        for x in data['test']
    ]
    ...
    y_score, _, y_test = list(zip(*scores))
    ...
    scores = [
        (strSimilarity.howConfusableAre(x[0].lower(), x[1].lower()), 1.0 - x[2])
        for x in data['test']
    ]

    y_score, y_test = list(zip(*scores))
